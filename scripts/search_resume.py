...
def start_wos(q, oa):
    ...
    submit = (DIR / 'wos_submit.js').read_text().replace('__QUERY__', json.dumps(quoted_query(q), ensure_ascii=False))
...
def metadata(args):
    urls = list(dict.fromkeys(line.strip() for line in Path(args.query).read_text().splitlines()
    ...
    for url in urls:
        ...
        if not obj:
            try:
                ...
                obj = json.loads(run_js(script.read_text()))
