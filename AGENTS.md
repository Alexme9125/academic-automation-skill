...
- Pure PubMed search/metadata use a separate data lock and per-task API handoffs. They may proceed while a browser task is paused, without changing that browser pending state. All NCBI API requests share a separate rate-limit lock.
