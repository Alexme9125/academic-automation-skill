...
流程：doi.org 跳转 → 出版社分流 → 可公开请求的真实 PDF → 已登录浏览器页内链接 → macOS Apple Events 尝试原生保存 / Windows 人工交接 → 核验归档。macOS 不使用 Playwright；浏览器路径复用当前会话，独立 HTTP 请求不导出用户 cookies。
...
已发起正文请求时保存 `publisher_stage`。用户验证后恢复，先检查文件，再读取当前页及同篇弹出页；同篇 PDF 已打开则从保存阶段继续，不返回文章页重复点击。macOS 使用 Apple Events 原生保存；Windows 的 Playwright 查看器自动保存尚未通过稳定性验收，未落盘时须询问“仅题录”或“保留标签页、批量手动下载”，执行[受阻后的选择](cli.md#自动保存受阻后的选择)。两端均保留目录恢复，不把缺少下载事件当成重点击授权。
