...
- PowerShell 5.1 与 7 分别通过 `py -3 scripts/academic.py` 和 `academic.cmd` 执行 UTF-8 查询文件；包含嵌套双引号、中文/空格路径及 BOM，核对 `query` 原样保留。
- PMID 42693903 的正文应通过身份核验；真实补充材料仍应暂停。报告中的固定声明不能再触发误判。
- PMID 41947537 使用 `--route pmc-only`，若现行服务仍无正文 PDF，应暂缓/仅题录且不索要 Token；后续 PMC 篇目继续。接口有新文件时应据实记录，不能强求旧状态。
- 另一篇浏览器任务等待时，独立 PMC 任务仍可运行；对同一暂停篇切换模式应被拒绝。
- 整批取消、重排后重跑、明确恢复分别核对状态，其他清单的待办不能被清除；保存完整清单与 PMC 子集的对账结果。
- 扩展连接、出版社自动保存、中文/空格/长路径以及人工恢复仍需真实 Windows Chrome 验证，离线用例和 macOS 数据服务测试不能替代。
